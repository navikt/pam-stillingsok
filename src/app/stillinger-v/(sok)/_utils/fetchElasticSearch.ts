...
import elasticSearchRequestBody from "@/app/stillinger-v/(sok)/_utils/elasticSearchRequestBody";
...
export async function fetchElasticSearch(query: SearchQuery, headers: HeadersInit, fetchOptions = {}) {
    ...
    // console.log(`elastSearchRequestBody: ${JSON.stringify(body)}`);
    ...
    console.info(`ElasticSearch request took ${res.headers.get("x-response-time")} ms`);
    if (!res.ok) {
        console.info(`Search query failed: ${res.status} - ${res.statusText}`);
    }
...
async function fetchSimplifiedElasticSearch(
    ...
    if (!response?.ok) {
        console.error(`Failed to fetch data from elastic search: ${response?.status}`);
