...
import {
  BodyLong, Button, HStack, Heading, Modal, VStack,
...
const SessionStatusModal = ({
  ...
  const handleSessionInfoResponse = async (response, isCurrentlyLoggedIn, errorMessage) => {
    if (response.status === 401) {
      ...
      const sessionIsTimingOut = session.timeout_in_seconds > -1
  ...
  return (
    ...
    >
      <Modal.Header closeButton={false}>
        <Heading className="mb-2" level="1" size="medium">
          {title}
        </Heading>
      </Modal.Header>
      <Modal.Body>
        <VStack gap="6">
          <BodyLong suppressHydrationWarning role="timer">
            {message}
          </BodyLong>
