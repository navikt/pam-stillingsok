...
const SelectedFilters = ({ query, queryDispatch }) => {
  ...
  const removeOccupation = (value) => {
    ...
    const remainingOccupationsInCategory = query.occupationSecondLevels.filter((secondLevel) => secondLevel.startsWith(`${firstLevel}.`));
  ...
  chips.push(
    ...query.municipals.map((value) => (
      <Chips.Removable key={`municipals-${value}`} variant="neutral" onClick={() => removeMunicipal(value)}>
  ...
  chips.push(
    ...query.countries.map((value) => (
      <Chips.Removable key={`countries-${value}`} variant="neutral" onClick={() => removeCountry(value)}>
        {fixLocationName(value)}
      </Chips.Removable>
    )),
  );
