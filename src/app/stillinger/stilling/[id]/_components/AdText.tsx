...
export default function AdText({ adText }: { adText: string }): ReactNode {
    const sanitizedAdText = adText ? sanitizeHtml(adText.trim()) : "";
    if (sanitizedAdText) {
        if (sanitizedAdText.includes("arb-aapningstekst")) {
            return <RichText className="job-posting-text">{parse(sanitizedAdText, options)}</RichText>;
        }
        return <RichText className="job-posting-text">{parse(sanitizedAdText, optionsReplaceHeadings)}</RichText>;
    }
    return null;
}
