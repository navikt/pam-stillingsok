...
export function transformAdDataLegacy(raw: unknown): Result<z.infer<typeof AdDTO>, ParseError> {
    ...
    const out = {
        ...
        application: {
            applicationDueDate, // IsoDateString | null
        ...
        extent: arrOrNull(toStringArray(properties?.extent)),
        workDays: arrOrNull(toStringArray(properties?.workday)),
        workHours: arrOrNull(toStringArray(properties?.workhours)),
        jobPercentage: orNull(jobPercentage ?? jobPercentageRange),
        workLanguages: arrOrNull(toStringArray(properties?.workLanguage)),
