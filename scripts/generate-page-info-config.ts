...
function stringifyPageInfoConfig(config: PageInfoConfig): string {
    const indent = "    ";
    const lines: string[] = [];
...
async function generatePageInfoConfig(): Promise<void> {
    ...
    const outputPath = path.join(ARTICLES_ROOT, "pageInfoConfig.generated.ts");
    const configString = stringifyPageInfoConfig(config);
...
export const pageInfoConfig: PageInfoConfig = ${configString};
